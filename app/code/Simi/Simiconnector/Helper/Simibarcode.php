...
class Simibarcode extends \Simi\Simiconnector\Helper\Data {
    ...
    public function generateCode($string) {
        ...
        $checkBarcodeExist = $this->_objectManager->create('Simi\Simiconnector\Model\Simibarcode')->load($barcode, 'barcode');
    ...
    public function getValueForBarcode($table, $column, $productId, $data) {
        if ($table == 'product') {
            ...
            $model = $this->_objectManager->create('Magento\Catalog\Model\Product')->load($productId);
    ...
    public function importProduct($data) {
        if (count($data)) {
            $this->_objectManager->create('Magento\Backend\Model\Session')->setData('null_barcode_product_import', 0);
        } else {
            $this->_objectManager->create('Magento\Backend\Model\Session')->setData('null_barcode_product_import', 1);
    ...
    {
        return $this->_objectManager->create('Magento\Framework\Math\Random')->getRandomString($length, $chars);
    }
}
