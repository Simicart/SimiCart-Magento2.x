...
class Productlist extends Data
{
    ...
    public function getProductCollection($listModel)
    {
        ...
        switch ($listModel->getData('list_type')) {
            //Product List
            case 1:
                $collection->addFieldToFilter('entity_id', ['in' => explode(',', $listModel->getData('list_products'))]);
                break;
            //Best seller
            case 2:
                ...
                $select = $collection->getSelect()
                        ->join(['order_item' => $orderItemTable], 'order_item.product_id = entity_id', ['order_item.product_id', 'order_item.qty_ordered'])
            ...
            case 3:
                ...
                $collection = $this->_objectManager->create('Magento\Catalog\Model\Product')->getCollection();
                $select = $collection->getSelect()
                        ->join(['product_viewed' => $productViewTable], 'product_viewed.product_id = entity_id', ['product_viewed.product_id', 'product_viewed.views_num'])
    ...
    public function getMatrixLayoutMockup($storeviewid, $controller)
    {
        ...
        $simicategoryCollection->getSelect()
                ->join(['visibility' => $visibilityTable], 'visibility.item_id = main_table.simicategory_id AND visibility.content_type = ' . $typeID . ' AND visibility.store_view_id =' . $storeviewid);
        ...
        $listCollection->getSelect()
                ->join(['visibility' => $visibilityTable], 'visibility.item_id = main_table.productlist_id AND visibility.content_type = ' . $listtypeID . ' AND visibility.store_view_id =' . $storeviewid);
    ...
    public function autoFillMatrixRowHeight()
    {
        ...
        foreach ($this->_objectManager->get('Simi\Simiconnector\Model\Simicategory')->getCollection() as $simicat) {
            $currentIndex = $simicat->getData('matrix_row');
            if (!isset($rows[$currentIndex])) {
                $rows[$currentIndex] = ['phone' => $simicat->getData('matrix_height_percent'), 'tablet' => $simicat->getData('matrix_height_percent_tablet')];
            }
        }
        foreach ($this->_objectManager->get('Simi\Simiconnector\Model\Productlist')->getCollection() as $productlist) {
            $currentIndex = $productlist->getData('matrix_row');
            if (!isset($rows[$currentIndex])) {
                $rows[$currentIndex] = ['phone' => $productlist->getData('matrix_height_percent'), 'tablet' => $productlist->getData('matrix_height_percent_tablet')];
            }
        }
        ksort($rows);
        $script = '
