        ...
        foreach ($storeViewCollection as $storeview) {
            ...
            if ($helper->getConfig('simi_notifications/notice_price/noti_price_enable', $storeviewId)) {
                ...
                if ($oldSpecialPrice != $newSpecialPrice && $newProduct->getId() > 0 && $newProduct->getStatus() == '1' && $newProduct->getVisibility() != '1') {
                    ...
                    $data['notice_url'] = $helper->getConfig('simi_notifications/notice_price/noti_price_url', $storeviewId);
                    $data['notice_content'] = $content;
                    $data['device_id'] = $helper->getConfig('simi_notifications/notice_price/noti_price_platform', $storeviewId);
                    $data['notice_sanbox'] = $helper->getConfig('simi_notifications/notice_price/noti_price_sandbox', $storeviewId);
                    $data['type'] = $helper->getConfig('simi_notifications/notice_price/noti_price_type', $storeviewId);
                    $data['product_id'] = $newProduct->getId();
                    $data['category_id'] = $helper->getConfig('simi_notifications/notice_price/noti_price_category_id', $storeviewId);
                    $data['category_name'] = $this->getCategoryName($helper->getConfig('simi_notifications/notice_price/noti_price_category_id', $storeviewId));
                    $data['has_child'] = $this->getCategoryChildrenCount($helper->getConfig('simi_notifications/notice_price/noti_price_category_id', $storeviewId));
        ...
        foreach ($storeViewCollection as $storeview) {
            ...
            if ($helper->getConfig('simi_notifications/noti_new_product/new_product_enable', $storeviewId)) {
                ...
                if ($newProduct->getId() && $newProduct->getId() == $lastProductId && $newProduct->getStatus() == '1' && $newProduct->getVisibility() != '1' && $newProduct->getSku() == $this->new_added_product_sku) {
                    ...
                    $data['notice_url'] = $helper->getConfig('simi_notifications/noti_new_product/new_product_url', $storeviewId);
                    $data['notice_content'] = $content;
                    $data['device_id'] = $helper->getConfig('simi_notifications/noti_new_product/new_product_platform', $storeviewId);
                    $data['notice_sanbox'] = $helper->getConfig('simi_notifications/noti_new_product/new_product_sandbox', $storeviewId);
                    $data['type'] = $helper->getConfig('simi_notifications/noti_new_product/new_product_type', $storeviewId);
                    $data['product_id'] = $newProduct->getId();
                    $data['category_id'] = $helper->getConfig('simi_notifications/noti_new_product/new_product_category_id', $storeviewId);
                    $data['category_name'] = $this->getCategoryName($helper->getConfig('simi_notifications/noti_new_product/new_product_category_id', $storeviewId));
                    $data['has_child'] = $this->getCategoryChildrenCount($helper->getConfig('simi_notifications/noti_new_product/new_product_category_id', $storeviewId));
                    $data['created_time'] = time();
                    $data['notice_type'] = 2;
                    $data['notice_sanbox'] = '2';
                    $data['storeview_id'] = $storeviewId;
