...
class Cms extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function getCategoryCMSPages()
    {
        ...
        $cmsCollection->getSelect()
                ->join(['visibility' => $visibilityTable], 'visibility.item_id = main_table.cms_id AND visibility.content_type = ' . $typeID . ' AND visibility.store_view_id =' . $storeManager->getStore()->getId());
        $cmsArray = [];
