...
class Productgrid extends \Magento\Backend\Block\Widget\Grid\Extended
{
    ...
    function __construct(
        ...
        \MobileApp\Connector\Model\BannerFactory $noticeFactory,
    ...
    {
        $notice_id = $this->getRequest()->getParam('id');
        if (!isset($tm_id)) {
            $notice_id = 0;
        }

        $notice = $this->_noticeFactory->create()->load($notice_id);
        $products = array();
        if($notice->getId()){
            $products = array($notice->getProductId());
        }
